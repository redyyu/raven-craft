...
-- end
local old_ISAttachItemHotbar_perform = ISAttachItemHotbar.perform

function ISAttachItemHotbar:perform()
	
	-- local sniper = tostring(self.slotDef.type) 
	local sniper = ( self.slotIndex ~= 1 )
	
	if not sniper
	or not self.hotbar.replacements[self.item:getAttachmentType()] 
	then
		old_ISAttachItemHotbar_perform(self)
		return
	end
	
	-- print("Slot " .. tostring(self.slot))
	-- print("Slot Def " .. tostring(self.slotDef))
	-- print("Slot Def Type" .. tostring(self.slotDef.type))
	...
	-- print("Slot Index " .. tostring(self.slotIndex))
	-- add new item
	-- if the item need to be attached elsewhere than its original emplacement because of a bag for example
	if self.hotbar.replacements and self.hotbar.replacements[self.item:getAttachmentType()] 
	-- and (not (sniper and self.slotIndex ~= 1))
	and not sniper
	-- and not self.slotIndex == 1
	then
		-- print("Bag Replacement")
	...
	-- if sniper and self.slotIndex ~= 1 then self.slot = "SniperPack" end
