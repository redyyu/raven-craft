...
local Ladder = require('Ladder/Ladder')
	...
	ISButtonPrompt.testBButtonAction = function(self,dir)
		if not self.bPrompt then
			...
			if hasClimbFlag and player:canClimbSheetRope(square) then
				self:setBPrompt(getText("UI_Climb"), GamePad.triggerGamepadClimbing, Joypad.BButton, square, false)
			else
				...
				if square and IsoWindow.isTopOfSheetRopeHere(square) and player:canClimbDownSheetRope(square) then
					self:setBPrompt(getText("UI_Climb"), GamePad.triggerGamepadClimbing, Joypad.BButton, square, true)
				end
			end
		end
