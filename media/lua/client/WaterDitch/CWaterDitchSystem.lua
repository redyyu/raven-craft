...
-- 	local ditch = CWaterDitchSystem.instance:getIsoObjectOnSquare(square)
-- 	if not ditch or not ditch:getModData()["waterMax"] then return end
...
-- 	tooltipUI:DrawTextCentre(getText("ContextMenu_Water_Ditch"), tooltipUI:getWidth() / 2, 6, 1, 1, 1, 1)
-- 	tooltipUI:DrawText(getText("IGUI_invpanel_Remaining") .. ":", textX, textY, 1, 1, 1, 1)

-- 	local f = ditch:getWaterAmount() / ditch:getModData()["waterMax"]
