...
Ditch.onFillWorldObjectContextMenu = function(playerNum, context, worldobjects)
    ...
    elseif JoypadState.players[playerNum+1] or ISWaterDitch.canDigHere(worldobjects) then
        ...
        if shovel then
            ...
            context:addSubMenu(ditchOption, icthMenu)
            local optPool = context:addOption(getText("ContextMenu_DigDitch_Pool"), worldobjects, Ditch.onDigDitch, playerNum, shovel, true)
            optPool.toolTip = ISWorldObjectContextMenu.addToolTip()
            optPool.toolTip.description = getText("Tooltip_DigDitch_Pool")
            local optWaterway = context:addOption(getText("Tooltip_DigDitch_Waterway"), worldobjects, Ditch.onDigDitch, playerNum, shovel)
